...
class FSDPParam:
    ...
    def _init_sharded_param(self, param: nn.Parameter):
        ...
        slice_list = [slice(None)] * param.ndim
        
        rank_size = param.size(shard_dim) // shard_world_size
        begin = rank_size * shard_rank
        slice_list[shard_dim] = slice(begin, begin + rank_size)
        # sharded_param = param[tuple(slice_list)]
        sharded_param = param[tuple(slice_list)].clone()
